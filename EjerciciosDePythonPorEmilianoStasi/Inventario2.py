...
#usamos para ver si el vector se encuentra vacio o lleno
def Buscar(Codigo):
...
#Actualizamos el Saldo del Producto deacuardo al movimientos (Ingreso/Egreso)
...
def Ingreso():
    ...
    if Nombre=='':
        Nombre=input("Ingrese la Descripcion del Producto: ")
    else:
        Nombre=BuscarNombre(Codigo)
    ...
    Orden=input("Ingrese el Nº de Orden de Compra: ")
    if Buscar(Codigo)==0:
...
def Egreso():
    ...
    if len(Vector)>0:
        ...
        Orden=input("Ingrese el Nº de Factura: ")
        VKARDEX.append("E")
        VKARDEX.append(Codigo)
...
def Kardex():
    ...
    if BuscarNombre(Codigo)=='':
        ...
        for VKARDEX in ListaKardex:
            if VKARDEX[1]==Codigo:
                print(VKARDEX[1],VKARDEX[2],VKARDEX[0],VKARDEX[3],VKARDEX[4],VAKARDEX[6],sep="\t")
    print('')
